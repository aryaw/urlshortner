...
func (m AuthModel) CreateAuth(userid int64, td *TokenDetails) error {
	...
	errAccess := config.GetRedis().Set(td.AccessUUID, strconv.Itoa(int(userid)), at.Sub(now)).Err()
	if errAccess != nil {
		return errAccess
	}
	errRefresh := config.GetRedis().Set(td.RefreshUUID, strconv.Itoa(int(userid)), rt.Sub(now)).Err()
	if errRefresh != nil {
		return errRefresh
	}
	return nil
...
func (m AuthModel) FetchAuth(authD *AccessDetails) (int64, error) {
	userid, err := config.GetRedis().Get(authD.AccessUUID).Result()
	if err != nil {
		return 0, err
	}
...
func (m AuthModel) DeleteAuth(givenUUID string) (int64, error) {
	deleted, err := config.GetRedis().Del(givenUUID).Result()
	if err != nil {
		return 0, err
	}
	return deleted, nil
